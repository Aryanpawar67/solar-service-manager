...
const CATEGORIES = ["Technical Issue", "Job Assignment Issue", "Equipment Problem", "Payment Query", "Leave Request", "Other"] as const;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/contact`, {
        ...
        body: JSON.stringify({
          name: meData?.user?.name ?? "Staff Member",
          email: meData?.user?.email ?? "staff@greenvolt.in",
          phone: "",
          message: `[${category}] ${message.trim()}`,
        }),
      });
      if (res.ok) {
        Alert.alert("Submitted", "Your request has been sent to the support team. We'll respond within 24 hours.", [
        ...
        <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.contactCard}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.contactSub}>Response within 24 hours on working days</Text>
            </View>
          </View>
